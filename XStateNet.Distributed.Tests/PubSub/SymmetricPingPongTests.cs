        ...
        public async Task SymmetricMachines_BidirectionalCommunication()
        {
            ...
            var machine1Actions = new Dictionary<string, Action<OrchestratedContext>>
            {
                ["onStart"] = ctx =>
                {
                    ...
                    ctx.RequestSend("machine2", "PING", machine1PingCount);
                },
                ["onPing"] = ctx =>
                {
                    var elapsedMs = (DateTime.UtcNow - testStartTime).TotalMilliseconds;
                    ...
                    ctx.RequestSend("machine2", "PONG", machine1PongCount);
            ...
            var machine2Actions = new Dictionary<string, Action<OrchestratedContext>>
            {
                ["onStart"] = ctx =>
                {
                    ...
                    ctx.RequestSend("machine1", "PING", machine2PingCount);
                },
                ["onPing"] = ctx =>
                {
                    var elapsedMs = (DateTime.UtcNow - testStartTime).TotalMilliseconds;
                    ...
                    ctx.RequestSend("machine1", "PONG", machine2PongCount);
            ...
            await _orchestrator.SendEventAsync("test", "machine1", "START");
            ...
            Assert.Equal("#machine1.done", machine1.CurrentState);
            Assert.Equal("#machine2.done", machine2.CurrentState);
        ...
        public async Task SymmetricMachines_SimultaneousInitiation()
        {
            ...
            var machine1Actions = new Dictionary<string, Action<OrchestratedContext>>
            {
                ["onStart"] = ctx =>
                {
                    ...
                    ctx.RequestSend("sym2", "MESSAGE", new { from = "sym1" });
                ...
                ["onMessage"] = ctx =>
                {
                    ...
                    if (machine1MessageCount < 3)
                    {
                        ...
                        ctx.RequestSend("sym2", "REPLY", new { from = "sym1" });
            ...
            var machine2Actions = new Dictionary<string, Action<OrchestratedContext>>
            {
                ["onStart"] = ctx =>
                {
                    ...
                    ctx.RequestSend("sym1", "MESSAGE", new { from = "sym2" });
                    ctx.RequestSelfSend("SENT");
                },
                ["onMessage"] = ctx =>
                {
                    ...
                    if (machine2MessageCount < 3)
                    {
                        ...
                        ctx.RequestSend("sym1", "REPLY", new { from = "sym2" });
                        ctx.RequestSelfSend("REPLY");
                    }
                    else
                    {
            ...
            // Create machines using orchestrated pattern
            var machine1 = CreateMachine("sym1", createMachine("sym1"), machine1Actions);
            var machine2 = CreateMachine("sym2", createMachine("sym2"), machine2Actions);
            ...
            await Task.WhenAll(
                _orchestrator.SendEventAsync("test", "sym1", "START"),
                _orchestrator.SendEventAsync("test", "sym2", "START")
            );
            ...
            Assert.Equal("#sym1.complete", machine1.CurrentState);
            Assert.Equal("#sym2.complete", machine2.CurrentState);
