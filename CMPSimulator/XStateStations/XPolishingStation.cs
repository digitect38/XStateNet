...
/// <summary>
/// Polishing Station using XStateNet state machine
/// States: idle → processing → ready
/// </summary>
public class XPolishingStation
{
    ...
    public XPolishingStation(string stationId, EventBusOrchestrator orchestrator)
    {
        ...
        {
            Log($"⚙️  Processing Wafer {_currentWaferId} (3000ms)");
        });

        AddAction("processingComplete", (sm) =>
        {
            Log($"✓ Wafer {_currentWaferId} processing complete");
        });
        ...
        AddAction("notifyReady", (sm) =>
        {
            ...
            {
                Log($"📤 Wafer {_currentWaferId} ready for pickup");

                // Request WTR2 to transfer to Cleaner
    ...
    {
        LogMessage?.Invoke(this, $"[Polisher] {message}");
    }
}

public class WaferArrivedEventArgs : EventArgs
{
    public int WaferId { get; }
    public string StationId { get; }

    public WaferArrivedEventArgs(int waferId, string stationId)
    {
        WaferId = waferId;
        StationId = stationId;
    }
...
public class WaferPickedUpEventArgs : EventArgs
{
    public int WaferId { get; }
    public string StationId { get; }

    public WaferPickedUpEventArgs(int waferId, string stationId)
    {
        WaferId = waferId;
        StationId = stationId;
    }
