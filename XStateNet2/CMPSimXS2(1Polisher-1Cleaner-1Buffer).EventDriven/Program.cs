...
Console.WriteLine("3 robots, 1 carrier (25 wafers), 1 polisher, 1 cleaner, 1 buffer");
Console.WriteLine();
...
try
{
    ...
    var jsonPath = Path.Combine(exeDir, "cmp_machine_p1c1b1.json");

    if (!File.Exists(jsonPath))
    {
        Console.Error.WriteLine($"[ERROR] State machine definition not found: {jsonPath}");
        Console.Error.WriteLine($"Looking in: {exeDir}");
        Console.Error.WriteLine("Please ensure cmp_machine_p1c1b1.json exists and is copied to output directory.");
        return;
    }
