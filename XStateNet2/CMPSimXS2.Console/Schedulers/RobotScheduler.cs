...
public class RobotScheduler : IRobotScheduler
{
    ...
    {
        Logger.Instance.Log("[RobotScheduler] Initialized");
    }
    ...
    public void RegisterRobot(string robotId, IActorRef robotActor)
    {
        lock (_lock)
        {
            ...
            Logger.Instance.Log($"[RobotScheduler] Registered robot: {robotId}");
        }
    }
    ...
    public void UpdateRobotState(string robotId, string state, int? heldWaferId = null, string? waitingFor = null)
    {
        lock (_lock)
        {
            ...
            {
                Logger.Instance.Log($"[RobotScheduler:WARNING] {robotId} cannot be idle while holding wafer {heldWaferId}! Clearing wafer.");
                heldWaferId = null;
            }
            ...
            Logger.Instance.Log($"[RobotScheduler:DEBUG] Robot {robotId} state updated: {state} (wafer={heldWaferId ?? 0})");
            ...
            if (state == "idle" && !wasIdle)
            {
                ...
                if (_activeTransfers.ContainsKey(robotId))
                {
                    ...
                    {
                        Logger.Instance.Log($"[RobotScheduler] {robotId} completed transfer of wafer {completedTransfer.WaferId}, invoking callback");
    ...
    public void RequestTransfer(TransferRequest request)
    {
        lock (_lock)
        {
            ...
            {
                Logger.Instance.Log($"[RobotScheduler:ERROR] Invalid transfer request: {ex.Message}");
                return;
            }
            ...
            if (assignedRobot == null)
            {
                ...
                Logger.Instance.Log($"[RobotScheduler] No robots available, queued: {request} (Queue size: {_pendingRequests.Count})");
            }
        }
    }
    ...
    private bool IsRobotAvailable(string robotId)
    {
        ...
        {
            Logger.Instance.Log($"[RobotScheduler:WARNING] {robotId} is idle but still holding wafer {robotState.HeldWaferId}! Clearing...");
    ...
    private void ExecuteTransfer(string robotId, TransferRequest request)
    {
        ...
        {
            Logger.Instance.Log($"[RobotScheduler:ERROR] Robot {robotId} not found");
            return;
        }

        Logger.Instance.Log($"[RobotScheduler] Assigning {robotId} for transfer: {request}");
        ...
        Logger.Instance.Log($"[RobotScheduler] Sent PICKUP to {robotId}: wafer {request.WaferId} from {request.From} to {request.To}");
    }
    ...
    private void ProcessPendingRequests()
    {
        while (_pendingRequests.Count > 0)
        {
            ...
            if (assignedRobot != null)
            {
                ...
                Logger.Instance.Log($"[RobotScheduler] Processed pending request: {request} assigned to {assignedRobot}");
            }
            else
            {
