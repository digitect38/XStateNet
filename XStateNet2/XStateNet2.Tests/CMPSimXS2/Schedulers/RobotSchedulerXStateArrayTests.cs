...
/// <summary>
/// Comprehensive tests for RobotSchedulerXStateArray - the array-optimized champion scheduler.
/// Tests all critical features: robot registration, state updates, transfer lifecycle,
/// robot selection strategies, immediate assignment, ActiveTransfers tracking, and queue processing.
/// </summary>
[Collection("Sequential")] // Disable parallel execution - Akka TestKit tests with timing dependencies
public class RobotSchedulerXStateArrayTests : TestKit
{
    private readonly RobotSchedulerXStateArray _scheduler;
    private readonly TestProbe _robot1;
    private readonly TestProbe _robot2;
    private readonly TestProbe _robot3;

    public RobotSchedulerXStateArrayTests()
    {
        _scheduler = new RobotSchedulerXStateArray(Sys, "test-array-scheduler");
    ...
    {
        // Arrange
        var scheduler = new RobotSchedulerXStateArray(Sys, "test-registration");
    ...
    {
        // Arrange
        var scheduler = new RobotSchedulerXStateArray(Sys, "test-multiple-robots");
    ...
    #region Concurrency and Actor Message Handling Tests
    ...
    [Fact]
    public void ActorMessageOrdering_ShouldMaintainConsistency()
    {
        // This test verifies that actor message handling maintains consistent state
        // even when multiple messages are sent in rapid succession
        ...
        // Assert - Messages should be processed in order, maintaining consistency
        AwaitAssert(() =>
        {
            // First request should have been assigned, second might be queued or processed
            _scheduler.GetQueueSize().Should().BeLessThanOrEqualTo(1);
        }, TimeSpan.FromSeconds(2));
    }
